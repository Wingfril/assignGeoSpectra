...
;###############################################################
	...
	sebgsub:'Off',$
     	sepside:'Top',$	
     	secontinue:0L,$
     	sedoorders:ptr_new([1]),$
     	sensamps:0L,$
     	senaps:0L, $
     	slit_info:ptr_new(1),$
	suggested: -1,$
     	guideimagepath:'blank'}
;###############################################################
...
; Menu bar
     ...
     TITLES=['Paths','Cals','Combine Images','Point Source', $
             'Extended Source', 'Spatial Extraction','Other','Eng.']
...
;##############################################################################
;  Spatial Extraction Base

      w.se_base = widget_base(work_base,$
                                    /column,$
                                    map = 0)

         row1_base = widget_base(w.se_base,$
                                 /row)
            ...
            col1_base = widget_base(row1_base,$
               ...
               row = widget_base(col1_base,$
                 ...
                 button = widget_button(row,$
                                        font=buttonfont,$
                                        VALUE='Guide Image',$
                                        UVALUE='Guide Image Button',$
                                        EVENT_PRO = 'xspextool_event')  
                                                   
                 guideimage = coyote_field2(row,$
                                            LABELFONT=buttonfont,$
                                            FIELDFONT=textfont,$
                                            TITLE=':',$
                                            UVALUE='Guide Image',$
                                            ...
                                            /CR_ONLY,$
                                            textID=textid)
                 w.guideimage_fld = [guideimage,textid]

               geom_button = widget_button(col1_base,$
                                           font=buttonfont,$
                                           VALUE='Assign Geometry',$
                                           UVALUE='Assign Geometry',$
                                           /align_center)
               button = widget_button(col1_base,$
                                      font=buttonfont,$
                                      VALUE='Suggest/Flip Exraction Region',$
                                      UVALUE='Extraction Region',$
                                      /align_center)
                                           
               w.secol2_base = widget_base(row1_base,$
                                              /column,$
                                              /base_align_center,$
                                              frame=1)
                                              
               label = widget_label(w.secol2_base,$
                                    VALUE='2.  Make Spatial Profiles',$
                                    font=buttonfont,$
                                    /align_center)               

               mkprofile_button = widget_button(w.secol2_base,$
                                                font=buttonfont,$
                                                VALUE='Go',$
                                                UVALUE='Make Spatial Profiles',$
                                                /align_center)
               ...
               label = widget_label(w.secol2_base,$
                                    font=buttonfont,$
                                    VALUE='3.  Choose Orders',$
                                    /align_center)
               
               w.seorder_bg = cw_bgroup(w.secol2_base,$
                                              FONT=font,$
                                              strtrim(1,2),$
                                              /NONEXCLUSIVE,$
                                              /COLUMN,$
                                              ...
                                              SET_VALUE=[1],$
                                              UVALUE='SE Orders')
               widget_control, w.seorder_bg,SENSITIVE=0

            
            secol3_base = widget_base(row1_base,$
                                      /column,$                                
                                      frame=1,$
                                      /base_align_center)
            
               label = widget_label(secol3_base,$
                                    font=buttonfont,$
                                    VALUE='4.  Extraction/BG Regions',$
                                    /align_center)
               base = widget_base(secol3_base)

                   w.seapfindman_base = widget_base(base,$
                                                          /column,$
                                                          /base_align_center,$
                                                          map=1) 

                   appos = coyote_field2(w.seapfindman_base,$
                                         LABELFONT=buttonfont,$
                                         FIELDFONT=textfont,$
                                         TITLE='Extraction Region',$
                                         UVALUE='SE Aperture Position',$
                                         VALUE='0.0-15.0',$
                                         XSIZE=10,$
                                         EVENT_PRO = 'xspextool_fields',$
                                         /CR_ONLY,$
                                         textid=textid)
                   w.seappos_fld = [appos,textid]  
               
               w.sebg_bgroup = cw_bgroup(secol3_base,$
                                               /row,$
                                               label_left='BG Sub:',$
                                               font=buttonfont,$
                                               ['On','Off'],$
                                               ...
                                               /no_release,$
                                               set_VALUE=1,$
                                               UVALUE='SE BG Subtraction')
               
               w.sebginfo_base = widget_base(secol3_base,$
                                                   /column,$
                                                   /base_align_right,$
                                                   SENSITIVE=0)
               
                  bg = coyote_field2(w.sebginfo_base,$
                                     LABELFONT=buttonfont,$
                                     FIELDFONT=textfont,$
                                     TITLE='Bg:',$
                                     UVALUE='SE Bg Regions',$
                                     VALUE='14.5-15',$
                                     XSIZE=15,$
                                     EVENT_PRO = 'xspextool_fields',$
                                     /CR_ONLY,$
                                     textid=textid)
                  w.sebg_fld = [bg,textid]
                  
                  bgfitorder = coyote_field2(w.sebginfo_base,$
                                             LABELFONT=buttonfont,$
                                             FIELDFONT=textfont,$
                                             TITLE='Bg Fit Degree:',$
                                             UVALUE='SE Bg Fit Order',$
                                             XSIZE=8,$
                                             VALUE='1',$
                                             EVENT_PRO = 'xspextool_fields',$
                                             /CR_ONLY,$
                                             textID=textID)
                  w.sebgfitorder_fld = [bgfitorder,textID]
                           
                  
;define aps button removed (cosmetic only?)
               
         button = widget_button(w.se_base,$
                                font=buttonfont,$
                                VALUE='Extract',$
                                UVALUE='SE Extract')
...
pro xspextool_menuevent,event
  ...
  common xspextool_state
    ...
    print, 'so this is what an event looks like.....'
    print, event
    help, event
    print, event.id
    print, event.top
...
;###########################################################################
  widget_control, state.w.se_base,	   MAP = 0
;###########################################################################
...
;###########################################################################
     'Spatial Extraction': begin
        
        state.w.base = 'Spatial Extraction'
        widget_control, state.w.se_base,         /MAP
        widget_control, state.w.imageinput_base, /MAP
        if state.r.filereadmode eq 'Index' then begin
           
           widget_control, state.w.oprefix_fld[0], /SENSITIVE
           widget_control, state.w.iprefix_fld[0],  /SENSITIVE
           widget_control, state.w.iprefix_fld[1], $
...
;###########################################################################
  ;this one separate to avoid hourglass cursor during geometry assignment
  if uvalue eq 'Assign Geometry' then begin
  	path = mc_cfld(state.w.guideimage_fld,7,CANCEL=cancel)
   	state.r.guideimagepath=path
   	img = readfits(path,header)
   	xspextool_procguide,state,img,header,wid
  endif
;###########################################################################
...
;#################################################################################
     'Extraction Region': begin
    	;use state for slith arc/pix
      	slith_arc = (state.r.slith_arc eq 0) ? 15. : state.r.slith_arc
       	slith_pix = (state.r.slith_pix eq 0) ? 100. : state.r.slith_pix
       	if state.r.suggested eq -1 then begin
         	edge1 = min(where(*state.r.lons eq *state.r.lons))*slith_arc/(slith_pix-1)
         	edge2 = max(where(*state.r.lons eq *state.r.lons))*slith_arc/(slith_pix-1)
            print, 'state.r.suggested'
            print, state.r.suggested
            print, 'edge1'
            print, edge1
            print, 'edge2'
            print, edge2
         	widget_control, state.w.seappos_fld[1], $
           	SET_VALUE=strtrim(edge1,2)+'-'+strtrim(edge2,2) 
         	state.r.suggested = 0
       	endif else begin
         	n=n_elements(*state.r.lons)
         	;flip lat and lon order
         	*state.r.lons = (*state.r.lons)[n-1-indgen(n)]
         	*state.r.lats = (*state.r.lats)[n-1-indgen(n)]
         	edge1 = min(where(*state.r.lons eq *state.r.lons))*slith_arc/(slith_pix-1)
         	edge2 = max(where(*state.r.lons eq *state.r.lons))*slith_arc/(slith_pix-1)
            print, 'state.r.suggested'
            print, state.r.suggested
            ...
            print, edge2
         	widget_control, state.w.seappos_fld[1], $
           	SET_VALUE=strtrim(edge1,2)+'-'+strtrim(edge2,2) 
        endelse
    print, '*state.r.tracecoeffs'
    help, *state.r.tracecoeffs
       end
;#################################################################################
...
;#################################################################################
    'Guide Image Button': begin

        path= dialog_pickfile(DIALOG_PARENT=state.w.xspextool_base,$
                             /MUST_EXIST,PATH=state.r.datapath)
        if path ne '' then begin
            widget_control,state.w.guideimage_fld[1],SET_VALUE = path
            img = readfits(path,header)
        endif
     end
;#################################################################################
...
;#################################################################################
     'SE BG Subtraction': begin

        state.r.sebgsub = event.value
        if event.value eq 'On' then widget_control, state.w.sebginfo_base,$
          /SENSITIVE
        if event.value eq 'Off' then widget_control, state.w.sebginfo_base,$
          SENSITIVE=0

      end

      'SE Extract': xspextool_seextract,CANCEL=cancel
    
      'SE Orders': begin

        z = where( (*state.r.orders) eq event.value)
        test = (*state.r.sedoorders)
        test[z] = event.select
        z = where(test eq 1,count)
        if count lt 1 then begin
            ok = dialog_message('Must select at least one Order.',$
                                /ERROR,DIALOG_PARENT=state.w.xspextool_base)
            widget_control, state.w.seorder_bg, $
              SET_VALUE=reverse(*state.r.sedoorders)
            goto, cont   
        endif else (*state.r.sedoorders) = test
    end
;#################################################################################
     else:
...
;#################################################################################
    ptr_free, state.r.lons
    ptr_free, state.r.lats
    ptr_free, state.r.slit_info
;#################################################################################
...
;##############################################################################
pro xspextool_seextract, CANCEL=cancel
  ...
  if state.r.filereadmode eq 'Filename' then begin
    ...
    print, 'in seextract : 2'
    range = mc_cfld(state.w.seappos_fld,7,CANCEL=cancel)
    if range ne '' then begin
      range = float(strsplit(range,'-',/extract))
      range = mc_crange(range,[0,state.r.slith_arc],'Pos',/KGE,/KLE,$
               WIDGET_ID=state.w.xspextool_base,CANCEL=cancel)
      if cancel then mc_setfocus, state.w.seappos_fld
      if cancel then return    
    endif else range = [0.,state.r.slith_arc]

    doorders = *state.r.sedoorders
    good     = where(*state.r.sedoorders eq 1,norders)

    mins = fltarr(norders)
    for i =0,norders-1 do begin
        x = findgen(xranges[1,i]-xranges[0,i]+1)+xranges[0,i]
        bot = poly(x,(*state.r.edgecoeffs)[*,0,i]) 
        top = poly(x,(*state.r.edgecoeffs)[*,1,i])
        mins[i] = min(top-bot)
    endfor
    appix = 1
    print, 'in seextract : 3'

    print, 'apsize_arc'
    print, apsize_arc
    print, 'state.r.slith_arc'
    print, state.r.slith_arc
    print, 'mins'
    print, mins
    apsize_arc = range[1]-range[0]
    minpix = floor(min(mins)*apsize_arc/state.r.slith_arc)
    naps = floor(minpix/appix)
    state.r.senaps=naps
    pos = findgen(state.r.senaps)*apsize_arc/state.r.senaps+range[0]
    *state.r.appos = rebin(pos + (pos[1]-pos[0])/2.,state.r.senaps,state.r.norders)

    s=size(*state.r.lons)
    if s[0] ne 0 then begin
      *state.r.lons = (*state.r.lons)[pos[0]*state.r.slith_pix/apsize_arc:$
                                  pos[0]*state.r.slith_pix/apsize_arc+naps-1]
      *state.r.lats = (*state.r.lats)[pos[0]*state.r.slith_pix/apsize_arc:$
                                  pos[0]*state.r.slith_pix/apsize_arc+naps-1]
    endif
    print, 'in seextract : 4'
    state.r.secontinue = 4

    ;options needed here?
    ximgtool,*state.d.workimage
    print, 'in seextract : 4.1'
    xspextool_setrace,state,CANCEL=cancel

    ;from xspextool_xsextract #################
    ;WE NEED PREVIOUS STEPS DONE FIRST/CHECKED FOR!!

    a = *state.r.appos
    size = float(a[1,0]-a[0,0])/2.
    apradius = replicate(size,state.r.senaps)
    print, 'in seextract : 4.2'
    apradius = mc_crange(apradius,[0,state.r.slith_arc],'Apradius',/KGE,/KLE,$
                  WIDGET_ID=state.w.xspextool_base,CANCEL=cancel)
    print, 'in seextract : 4.3'
    if cancel then return
    print, 'in seextract : 5'
    ;BGSUB STUFF START
    if state.r.sebgsub eq 'On' then begin
    ...
    ;  If filename mode, check to make sure the outname is filled in.
    print, 'in seextract : 6'
    filename = (state.r.filereadmode eq 'Filename') ? 1:0
    if filename then file = mc_cfld(state.w.outname_fld,7,/EMPTY,CANCEL=cancel)
    if cancel then return

    ;  Extract spectra

    z = where(*state.r.sedoorders eq 1, norders)

    workimage = *state.d.workimage
    varimage  = *state.d.varimage
    print, 'in seextract : 7'
    xspextool_message, 'Extracting spectra...'
;---------------------
;======
    if state.r.fixbdpx[0] or state.r.optext[0] then begin

        idx=0
     c = (state.r.medprofile eq 1) ? *state.r.aveprofcoeffs:*state.r.profcoeffs
        for i = 0,state.r.norders-1 do begin
            if (*state.r.sedoorders)[i] eq 1 then begin
    ...
    print, '*state.r.tracecoeffs'
    help, *state.r.tracecoeffs
    spectra = zmo_extsespec(workimage,varimage, *state.d.wavecal, (*state.r.edgecoeffs)[*,*,z],$
                         *state.r.tracecoeffs,state.r.norders,state.r.senaps,$
                         (*state.r.xranges)[*,z],state.r.slith_arc,$
                         apradius,appix,BGR=bgr,BGORDER=bgfit,$
                         SPATCOEFFS=scoeff,BDPIXTHRESH=bdpxthresh, $
                         BGSUBIMG=bgsubimg,ERRORPROP=state.r.errorprop, $
                         BDPXMK=bdpxmk,BITMASK=*state.d.bitmask, /UPDATE,$
                         WIDGET_ID=state.w.xspextool_base,CANCEL=cancel,$
...
;--------------------
    ...
    state.r.secontinue=6

    ntags = n_tags(spectra)
    sizes = lonarr(2,ntags)
  
    for i = 0,ntags-1 do sizes[*,i] = size(spectra.(i),/DIMEN)
  
    spec = dblarr(max(sizes),4,ntags)*!values.f_nan

    for i = 0,ntags-1 do begin
     
        spec[0:(sizes[0,i]-1),*,i] = spectra.(i)

    endfor
  *state.d.spectra = spec

    ;xspextool_wavecal,state,CANCEL=cancel ### start
    ;cancel = 0
    ...
    ;BGSUB STUFF START
    if state.r.sebgsub eq 'On' then begin
        bgr  = mc_cfld(state.w.sebg_fld,7,CANCEL=cancel,/EMPTY)
        if cancel then return
        bgfit = mc_cfld(state.w.sebgfitdeg_fld,3,CANCEL=cancel,/EMPTY)
        if cancel then return
        bgfit = mc_crange(bgfit,[0,7],'Background Fit Order',/KGE,/KLE,$
                   WIDGET_ID=state.w.xspextool_base,CANCEL=cancel)
        if cancel then setfocus,state.w.sebgfitdeg_fld
        if cancel then return
    ...
    if state.r.wavecal then begin
        ...
        wavecal = mc_cfld(state.w.wavecal_fld,7,CANCEL=cancel,/EMPTY)
        if cancel then return
    ;  Read resolution
    ...
    help,state.r.guideimagepath
    print,state.r.guideimagepath
    print, 'opaths[0]'
        print, opaths[0]

    writespec_se,*state.d.spectra,opaths[0],aimage, $
      sky,flat,naps,orders,hdrinfo,appos,apradius, $
      state.r.modename,state.r.slith_pix,state.r.slith_arc, $
      state.r.slitw_pix,state.r.slitw_arc,*state.r.lons,*state.r.lats,$
      xunits,yunits,xtitle,ytitle,state.w.version,*state.r.slit_info,state.r.guideimagepath,$
      BGORDER=bgorder,BGR=bgr,TRACEFILE=tracefile, $
      WAVECAL=wavecal,DISP=disp,TEXT=text,FITS=fits, $
      LINEARITY=state.r.lc,EXT2D=state.r.e2d
        
    xvspec,opaths[0]+'.fits'
    
    ;xspextool_writespec ### finish

    if cancel then return

    ;end of xspextool_xsextract ###############

    xspextool_message, 'SE Extraction Complete.'
...
;##############################################################################
     if state.w.base eq 'Spatial Extraction' then begin
     ;SPATIAL EXTRACTION FULL ROUTINE SEQUENCE

       xspextool_seextract, CANCEL=cancel
       if cancel then return
     endif
;##############################################################################
...
;##############################################################################
pro xspextool_procguide,state,img,header,wid
  ;find slit
  ...
  ;vertical direction
  ;can we do a proximity-to-edge thing to favor close minima?

  pxsc = sxpar(header,'PLATE_SC')
  if state.r.slith_arc eq 0. then slth_arc=15. else slth_arc=state.r.slith_arc
  n_sltpix = slth_arc/pxsc
  if state.r.slith_pix eq 0. then slith_pix=100 else slith_pix=state.r.slith_pix
  if state.r.slitw_arc eq 0 then slitw_arc=0.3 else slitw_arc=state.r.slitw_arc

  ;what I have done here with pointers is disgusting. if anyone knows how to conveniently retrieve data
  ;from a separate widget app, please change.
  ;n_sltpix is number of pixels covered by slit in guide image
  ;slith_pix is number of pixels covered by slit in spectral image
  ;change these names to make sense!
  xgeom,img,HEADER=header,LAT_PTR=lat_ptr,LON_PTR=lon_ptr,SLIT_PTR=slit_ptr,N_SLTPIX=n_sltpix,SLITH_PIX=slith_pix,SLITW_ARC=slitw_arc,$
      GROUP_LEADER=state.w.xspextool_base
  slit_info=*slit_ptr
  lon=*lon_ptr
  lat=*lat_ptr

;WHYYYYY are we doing the same thing twice???? -a 2017 summer intern
  state.r.slit_info=slit_ptr
  state.r.lons=lon_ptr
  state.r.lats=lat_ptr

  ;suggest planet edge and BG region
  ;this is now wrong. needs to happen after lons are transferred. ie from another xspextool event.
  *state.r.slit_info = slit_info
  *state.r.lons = lon
  *state.r.lats = lat
  print, lon
  siz = size(lon, /DIMENSIONS)  
    ;if lon[0] ne 0 and lon[3] ne 0 then begin
    ;    state.w.finishgeo = 1
    ;endif
end
;##############################################################################
...
;##############################################################################
pro xspextool_setrace,state,CANCEL=cancel

  cancel = 0

  if state.r.secontinue lt 3 then begin
  ...
  start = mc_cfld(state.w.start_fld,3,/EMPTY,CANCEL=cancel)
  if cancel then return
  start = mc_crange(start,[0,state.r.ncols-1],'Start',/KGE,/KLE,$
               WIDGET_ID=state.w.xspextool_base,CANCEL=cancel)
  if cancel then return

  stop = mc_cfld(state.w.stop_fld,3,/EMPTY,CANCEL=cancel)
  if cancel then return
  stop = mc_crange(stop,[0,state.r.ncols-1],'Stop',/KGE,/KLE,$
              WIDGET_ID=state.w.xspextool_base,CANCEL=cancel)
  if cancel then return
  ...
  fitorder = mc_cfld(state.w.tracedeg_fld,3,/EMPTY,CANCEL=cancel)
  if cancel then return
  fitorder = mc_crange(fitorder,[0,8],'Fit Order',/KGE,/KLE,$
              WIDGET_ID=state.w.xspextool_base,CANCEL=cancel)
  if cancel then return
  ...
  z = where(*state.r.sedoorders eq 1,norders)
  *state.r.tracecoeffs = mc_aptotrace1d((*state.r.edgecoeffs)[*,*,z],$
...
;  Get user inputs.
  ...
  files = mc_fsextract(files,INDEX=index,FILENAME=filename, $
                       WIDGET_ID=state.w.xspextool_base,CANCEL=cancel)
  print, 'in xspextool_load1image, files'
  print, files
  help, files
  if cancel then return
...
print, state.r.suffix
  fullpaths = mc_mkfullpath(state.r.sourcepath,*state.r.workfiles,INDEX=index,$
                            ...
                            CANCEL=cancel)
  print, 'xspextool_loadimages, fullpaths'
  print, fullpaths
...
print, 'Hellos from inside load images; slith_arc is'
print, slith_arc
...
;##############################################################################
 	widget_control, state.w.secol2_base, UPDATE=0
  	widget_control, state.w.seorder_bg, /DESTROY
;##############################################################################
        mc_getfonts,font
        ...
        *state.r.xsdoorders = intarr(state.r.norders)+1
        ...
        if n_elements(labels) gt 10 then begin
           ...
           state.w.xsorder_bg = cw_bgroup(state.w.xscol2_base,$
                                          ...
                                          UVALUE='XS Orders')
;##############################################################################
	    state.w.seorder_bg = cw_bgroup(state.w.secol2_base,$
                                    	  FONT=font,$
                                   	  labels,$
                                   	  /NONEXCLUSIVE,$
                                   	  /COLUMN,$
...
;##############################################################################
   	widget_control, state.w.secol2_base, /UPDATE   
;##############################################################################
...
;  Now check mode for wavelength solution
     ...
     if keyword_set(CLEAR) then begin
        ...
        *state.r.xsdoorders = intarr(state.r.norders)+1
;##############################################################################
	    *state.r.sedoorders = intarr(state.r.norders)+1
...
;##############################################################################
        ...
        widget_control,state.w.seorder_bg,SET_VALUE=*state.r.sedoorders
...
pro xspextool_mkprofiles,BEEP=beep,CANCEL=cancel
  ...
  cancel = 0
;##############################################################################     
  if state.r.pscontinue lt 1 or state.r.xscontinue lt 1 or state.r.secontinue lt 1 then begin
;##############################################################################     
...
;############################################################################## 
     'Spatial Extraction': widget_control, state.w.seappos_fld[1],SET_VALUE=''
;############################################################################## 
...
;******************************************************************************
;
pro drm_xspextool,instrument,FAP=fap,ENG=eng,IPREFIX=iprefix,CANCEL=cancel
...
;
; Copied from mostly spextool_loadimage, but this allow us to modify states 
; from outside the program 
;
pro spextool_zmo_loadImage, flatPath, wavecalPath, fullpaths, reductionmode, $
                            files, CLEAR=clear, BEEP = beep

  common xspextool_state
  *state.r.workfiles = files
  state.r.reductionmode = reductionmode
       xspextool_message, 'Loading Flat Field...'
     mc_readflat,flatpath,flat,ncols,nrows,modename,ps,slith_pix,slith_arc, $
                 slitw_pix,slitw_arc,rp,norders,orders,edgecoeffs,xranges, $
                 rms,rotation,OMASK=omask,CANCEL=cancel
...
; Allows us to input guide image.
pro zmo_inputGeometry, geopath
    common xspextool_state
    print, geopath
    state.r.guideimagepath = geopath
    latlon = read_csv(geopath+'.a.reftable', HEADER = latlonHeader, $ 
                   N_TABLE_HEADER = 3, TABLE_HEADER = latlonTableHeader)
    *state.r.lats = latlon.field1
    *state.r.lons = latlon.field2
    *state.r.slit_info = latlon.field3
    ;img = readfits(geopath,header)

   	;xspextool_procguide, state, img, header, wid
        
    ;while state.w.finishgeo eq 0 do begin
        ;wait, 1
        ;state.w.finishgeo = 0
    ;ENDWHILE
    ;    state.w.finishgeo = 1
...
print, *state.r.lons
    ...
    print, edge2
        ...
        *state.r.lats = (*state.r.lats)[n-1-indgen(n)]
        ...
        SET_VALUE=strtrim(edge1,2)+'-'+strtrim(edge2,2) 
        endelse
    xspextool_mkprofiles,/BEEP
    xspextool_seextract, CANCEL = cancel
end

;##############################################################################
pro xspextool_zmo_procguide, img, header, outputPath
   common xspextool_state
  ;find slit
  ...
  zmo_xgeom, img, outputPath, HEADER=header, LAT_PTR=lat_ptr, $
          LON_PTR=lon_ptr, $
