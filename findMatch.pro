...
pro findMatch, guideRefPath, specRefPath, outputName, outputPath 	
	   ...
	   (guideTimes[0] gt spectraTimes[spectraNumber[0]-1])) then begin
		...
		for i = 0, spectraNumber[0] - 2, 2 do begin
			...
			if guideCount+2 lt maxGuideCount then begin
				if guideTimes[guideCount+1] - guideTimes[guideCount] $
                   lt 20 then begin
					guideCount += 1
				endif
			endif
